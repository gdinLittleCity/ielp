   ...
   <%@include file="../header.jsp" %>
    ...
    <div class="container">
    	<div class="showlocal">
        	...
        	<a href="${pageContext.request.contextPath }/show/identifyList.action">信息甄别</a></span>
            <hr>
        </div>
        <!--  列表  -->
        <div class="con">
        	<ul>
        		<c:forEach items="${pageBean.beanList }" var="identify">
        ...
        <c:choose>
					...
					<c:otherwise>
						<a class="page_turn" href="${pageContext.request.contextPath }/show/identifyList.action?pageNum=${pageBean.pageNum-1}">上一页</a>
					</c:otherwise>
				</c:choose>
				...
				<c:forEach begin="${begin}" end="${end}" var="i">
					<c:choose>
						...
						<c:otherwise>
						 <a href="${pageContext.request.contextPath }/show/identifyList.action?pageNum=${i}">${i }</a>
						</c:otherwise>
					</c:choose>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="page_turn" href="${pageContext.request.contextPath }/show/identifyList.action?pageNum=${pageBean.pageNum+1}">下一页</a>
					</c:otherwise>
				</c:choose>
            	 跳转到：
...
<script type="text/javascript">
	function go(){
		...
		location = "${pageContext.request.contextPath }/show/identifyList.action?pageNum="+pageNum;
