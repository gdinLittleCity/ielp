...
<div id="wrap">
   ...
   <%@include file="../header.jsp" %>
    ...
    <div class="container">
        ...
        <div class="con">
        	<ul>
        		<c:forEach items="${pageBean.beanList }" var="quantity">
                <li><a href="${pageContext.request.contextPath }/show/articleInfo.action?nid=${quantity.nid}">${quantity.title }</a><span>${quantity.publish_time}</span></li>
                </c:forEach>
            </ul>
        </div>
        <!--  页码  -->
        ...
        <c:choose>
					...
					<c:otherwise>
						<a class="page_turn" href="${pageContext.request.contextPath }/show/quantityList.action?pageNum=${pageBean.pageNum-1}">上一页</a>
					</c:otherwise>
				</c:choose>
				...
				<c:forEach begin="${begin}" end="${end}" var="i">
					<c:choose>
						...
						<c:otherwise>
						 <a href="${pageContext.request.contextPath }/show/quantityList.action?pageNum=${i}">${i }</a>
						</c:otherwise>
					</c:choose>
				...
				<c:choose>
					...
					<c:otherwise>
						<a class="page_turn" href="${pageContext.request.contextPath }/show/quantityList.action?pageNum=${pageBean.pageNum+1}">下一页</a>
					</c:otherwise>
				</c:choose>
            	 跳转到：
...
<script type="text/javascript">
	function go(){
		...
		location = "${pageContext.request.contextPath }/show/quantityList.action?pageNum="+pageNum;
